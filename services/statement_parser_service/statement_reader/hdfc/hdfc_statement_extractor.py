...
class HDFCStatementExtractor:
    """
        This class is responsible for extracting the statement of HDFC Bank.
    """
    ...
    def feeder(self, pages: list[list[str]]) -> list[Transaction]:
        ...
        """
        transactions = []
        self._get_opening_balance(pages[-1]) 
    ...
    def _get_opening_balance(self, page: list[list[str]]) -> float:
        ...
        for row in page:
            row = normalize_row(row).lower()
            
            if OPENING_BALANCE in previous_row:
                rows = clean_rows(row)
    ...
    def _build_transaction(self, transaction: dict, page_number: int, transaction_number: int) -> Transaction:
        ...
        """
        try:
            main_row = clean_rows(transaction['main_row'])
            ...
            for row in rows:
                ...
                if is_transaction_row(row, DATE_REGEX):
                    ...
                    if current_transaction['main_row']:
                        txn = self._build_transaction(current_transaction, page_number=page_no, 
                ...
                if found_bad_row or is_bad_row(row, BAD_ROW_KEYWORDS):
                    ...
                    if (
                        current_transaction
                        and previous_row
                        and any("From :" in cell for cell in previous_row)
                    ):
                        found_bad_row = False

                if (
                    current_transaction['main_row']
                    and current_transaction['main_row'] != row
                    and not found_bad_row
                ):
